...
@RestController
@RequestMapping(path = "/Inplace_Cambio_Stato_Fascicolo")
public class CambioStatoFascicolo {
	...
	@RequestMapping(value = "/Inplace_CambioStatoFascicolo", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	public String issueNewVoucher(@Valid @RequestBody Cambiostatofascicolobean body) throws JsonProcessingException {
		...
		/*	String receiverRole = multichainUtils.getRoleForAddress(body.getAddress_to());
			...
			*/
			...
			customFields.addProperty("statoOld", body.getStatoOld());
			customFields.addProperty("statoNew", body.getStatoNew());
			customFields.addProperty("idFascicolo", body.getIdFascicolo());
			customFields.addProperty("descrizione", body.getDescrizione());
			customFields.addProperty("tipo_oprazione", "Cambio Stato Fascicolo");
