...
import com.vantea.hoperay.multichainwrapper.services.beans.AssetParamsJsonCancellazione;
...
import com.vantea.hoperay.multichainwrapper.services.beans.input.InplaceCancellazionebean;
...
@RestController
@RequestMapping(path = "/Inplace_Cancellazione")
public class InplaceCancellazione {
	...
	@RequestMapping(value = "/Inplace_CancellazioneMessaggio", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	public String issueNewVoucher(@Valid @RequestBody InplaceCancellazionebean body) throws JsonProcessingException {
...
//			customFields.addProperty("value", body.getValue());
			...
			if (body.isStatoCancellazione() == true ) {
				customFields.addProperty("statoCancellazione", "Non Letto");
				
			}else {
				
				customFields.addProperty("statoCancellazione", "Ripristinato");
			}
			
			customFields.addProperty("descrizione", body.getDescrizione());
			customFields.addProperty("tipo_oprazione", "Cancellazione Messaggio Fascicolo");
			log.debug("Custom Fields: " + customFields.toString());
			AssetParamsJsonCancellazione assetCustomFields = gson.fromJson(customFields.toString(),AssetParamsJsonCancellazione.class);
	...
	@RequestMapping(value = "/Inplace_CancellazioneAlert", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	public String issueNewVoucherAlert(@Valid @RequestBody InplaceCancellazionebean body) throws JsonProcessingException {
		log.trace("----- Issue.issueNewVoucher -----");
		log.trace("BODY: " + body.toString());
...
//			customFields.addProperty("value", body.getValue());
			...
			customFields.addProperty("descrizione", body.getDescrizione());
			customFields.addProperty("tipo_oprazione", "Cancellazione Alert Fascicolo");
			log.debug("Custom Fields: " + customFields.toString());
			AssetParamsJsonCancellazione assetCustomFields = gson.fromJson(customFields.toString(), AssetParamsJsonCancellazione.class);
